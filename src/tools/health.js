...
export function registerHealthTools(server) {
  ...
  server.tool('tv_discover', 'Report which known TradingView API paths are available and their methods', {}, wrapTool(() => core.discover()));

  server.tool('tv_ui_state', 'Get current UI state: which panels are open, what buttons are visible/enabled/disabled', {}, wrapTool(() => core.uiState()));
  ...
  server.tool('tv_launch', 'Launch TradingView Desktop with Chrome DevTools Protocol (remote debugging) enabled. Auto-detects install location on Mac, Windows, and Linux, including Windows MSIX/Store installs. If a Store install blocks the debug port, automatically relaunches from a local package copy (result then includes msix_local_copy: true; the first fallback launch copies ~330MB one time, so it can take a minute).', {
    ...
    kill_existing: z.coerce.boolean().optional().describe('Kill existing TradingView instances first (default false — set true to opt in)'),
  }, wrapTool(({ port, kill_existing }) => core.launch({ port, kill_existing })));

  server.tool('tv_update', 'Update this MCP server to the latest version: git fast-forward of origin/main + npm ci when dependencies changed. Safe by design — refuses on non-git installs, dirty working trees, non-main branches, or diverged history. After a successful update the MCP server must be restarted to load the new code.', {}, wrapTool(() => update({})));
}
