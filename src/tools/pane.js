...
export function registerPaneTools(server) {
  server.tool('pane_list', 'List all chart panes in the current layout with their symbols and active state', {}, wrapTool(() => core.list()));
  ...
  }, wrapTool(({ layout }) => core.setLayout({ layout })));
  ...
  }, wrapTool(({ index }) => core.focus({ index })));
  ...
  }, wrapTool(({ index, symbol }) => core.setSymbol({ index, symbol })));
}
