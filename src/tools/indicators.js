...
export function registerIndicatorTools(server) {
  ...
  }, wrapTool(({ entity_id, inputs }) => core.setInputs({ entity_id, inputs })));
  ...
  }, wrapTool(({ entity_id, visible }) => core.toggleVisibility({ entity_id, visible })));
  ...
  }, wrapTool(({ query, limit }) => core.searchStudies({ query, limit })));
  ...
  }, wrapTool(({ query, match, section }) => core.addStudyFromSearch({ query, match, section })));
}
