...
export function registerDrawingTools(server) {
  ...
  }, wrapTool(({ shape, point, point2, overrides, text }) => core.drawShape({ shape, point, point2, overrides, text })));

  server.tool('draw_list', 'List all shapes/drawings on the chart', {}, wrapTool(() => core.listDrawings()));

  server.tool('draw_clear', 'Remove all drawings from the chart', {}, wrapTool(() => core.clearAll()));
  ...
  }, wrapTool(({ entity_id }) => core.removeOne({ entity_id })));
  ...
  }, wrapTool(({ entity_id }) => core.getProperties({ entity_id })));
}
