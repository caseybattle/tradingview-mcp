...
export function registerPineTools(server) {
  server.tool('pine_get_source', 'Get current Pine Script source code from the editor', {}, wrapTool(() => core.getSource()));
  ...
  }, wrapTool(({ source }) => core.setSource({ source })));

  server.tool('pine_compile', 'Compile / add the current Pine Script to the chart', {}, wrapTool(() => core.compile()));

  server.tool('pine_get_errors', 'Get Pine Script compilation errors from Monaco markers', {}, wrapTool(() => core.getErrors()));

  server.tool('pine_save', 'Save the current Pine Script (Ctrl+S)', {}, wrapTool(() => core.save()));

  server.tool('pine_get_console', 'Read Pine Script console/log output (compile messages, log.info(), errors)', {}, wrapTool(() => core.getConsole()));

  server.tool('pine_smart_compile', 'Intelligent compile: detects button, compiles, checks errors, reports study changes', {}, wrapTool(() => core.smartCompile()));
  ...
  }, wrapTool(({ type }) => core.newScript({ type })));
  ...
  server.tool('pine_list_scripts', 'List saved Pine Scripts', {}, wrapTool(() => core.listScripts()));
  ...
  }, wrapTool(({ source }) => core.analyze({ source })));
  ...
  }, wrapTool(({ source }) => core.check({ source })));
}
